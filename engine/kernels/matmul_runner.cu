...
int main() {
    ...
    CUDA_CHECK(cudaMalloc(&d_C_cublas, sizeC * sizeof(float)));
    ...
    CUDA_CHECK(cudaMemcpy(d_C_cublas, h_C_cublas, sizeC * sizeof(float), cudaMemcpyHostToDevice));

    dim3 block(16, 16);
    dim3 grid((N + block.x - 1) / block.x,
              (M + block.y - 1) / block.y);

    float ms1 = Engine::timeKernel([&]() {
        basic_sgemm<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    });

    float ms2 = Engine::timeKernel([&]() {
        run_cublas_sgemm(d_A, d_B, d_C_cublas, 1.0f, 1.0f, M, N, K);
    });

    CUDA_CHECK(cudaMemcpy(h_C, d_C, sizeC * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_C_cublas, d_C_cublas, sizeC * sizeof(float), cudaMemcpyDeviceToHost));

    std::cout << "basic_sgemm time: " << ms1 << " ms\n";
    std::cout << "cublasSgemm time: " << ms2 << " ms\n";

    Engine::verifyMatrix(h_C, h_C_cublas, M, N, error_log_file);

    delete[] h_A; delete[] h_B; delete[] h_C; delete[] h_C_cublas;
